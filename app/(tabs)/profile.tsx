import { dummyChats, dummyStudyKits, dummyUserProfile } from '@/data/dummyData';
import {
  Button,
  ButtonText,
  Divider,
  ...
  const stats = [
    {
      ...
      label: 'Study Kits',
      value: dummyStudyKits.length,
      color: 'bg-indigo-50'
    },
    {
      icon: <TrendingUp size={20} color="#10B981" />,
      label: 'Active Chats',
      value: dummyChats.length,
      color: 'bg-green-50'
    },
    {
      icon: <Target size={20} color="#F59E0B" />,
      label: 'Total Assets',
      value: dummyStudyKits.reduce((sum, kit) => sum + kit.assets.length, 0),
      color: 'bg-yellow-50'
    },
    {
  ...
  <TouchableOpacity onPress={onPress} className="bg-white">
    <View className="px-6 py-4 flex-row items-center justify-between">
      <View className="flex-row items-center flex-1">
        <View className={`p-2 rounded-lg mr-4 ${isDestructive ? 'bg-red-100' : 'bg-gray-100'}`}>
          {icon}
        </View>
        <View className="flex-1">
          <Text className={`font-medium ${isDestructive ? 'text-red-600' : 'text-gray-900'}`}>
            {title}
          </Text>
          {subtitle && (
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="bg-white shadow-sm border-b border-gray-200">
        <View className="px-6 py-4 flex-row items-center justify-between">
          <Heading className="text-gray-900 text-2xl">
            Profile
          </Heading>
        ...
        <View className="mt-4">
          ...
          <View className="bg-white">
            ...
            <SettingsOption
              ...
              onPress={() => Alert.alert('Study Preferences', 'Study preference settings would be here.')}
            />
          </View>
        </View>
